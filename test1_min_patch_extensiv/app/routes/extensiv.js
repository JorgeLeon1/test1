...
// Simple debug: confirm router & envs
...
// NEW: token probe — verifies OAuth works
r.get('/token', async (_req, res) => {
  try {
    const h = await authHeaders(); // forces token fetch
...
// NEW: shotgun ping — tries common header/path combos
r.get('/ping2', async (_req, res) => {
  ...
  for (const c of combos) {
    ...
    try {
      ...
      results.push({ url, headerSet: c.label, status: resp.status, ok: true, sample: resp.data?.data?.[0] || resp.data });
      return res.json({ tried: results, winner: { url, headerSet: c.label } });
    } catch (e) {
      results.push({
        url, headerSet: c.label, ok: false,
        status: e.response?.status || null,
        data: e.response?.data || String(e.message)
      });
    }
  }
  res.status(500).json({ tried: results });
});
