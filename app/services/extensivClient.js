...
const s = (v, max = 255) =>
  v == null ? "" : String(v).normalize("NFC").slice(0, max);
...
export async function fetchOneOrderDetail(orderId) {
  ...
  // Fallback: just grab page 1 and find
  try {
...
async function upsertOrderDetail(pool, cols, rec) {
  ...
  const fieldDefs = [
    // [columnName, paramName, type, value]
    ...
    ["ItemID", "ItemID", sql.VarChar(150), s(rec.SKU, 150)], // mirror SKU if ItemID exists
  ...
  // include only columns that exist in dbo.OrderDetails
 ...
 *   openOnly (default true) -> only status=Open & not fully allocated
 */
export async function fetchAndUpsertOrders({ maxPages = 10, pageSize = 200, openOnly = true } = {}) {
  ...
  for (let pg = 1; pg <= maxPages; pg++) {
    ...
    for (const ord of orders) {
      ...
      // If no items were included, fetch detail for this order and retry
      let lines = items;
      if (!lines.length) {
        try {
      ...
      for (const it of lines) {
        ...
        const rec = {
          ...
          SKU: sku, // will also copy into ItemID if that column exists
          Qualifier: qualifier,
          OrderedQTY: qty,
          UnitID: unitId,
          UnitName: unitName,
    ...
    if (orders.length < pageSize) break; // last page reached
  }
